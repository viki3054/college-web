...
export default function TeacherHomeworkForm() {
  ...
  async function submit() {
    ...
    const res = await fetch("/api/teacher/homework", { method: "POST", body: fd });
    const j = await res.json().catch(() => ({}));
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Explain the task and submission instructions."
        />
      </div>
      ...
      <button
        disabled={!classId || title.length < 3 || description.length < 5 || saving}
        onClick={submit}
